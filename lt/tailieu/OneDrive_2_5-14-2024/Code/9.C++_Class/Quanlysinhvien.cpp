...
void _flush(){
    // See more at: https://stackoverflow.com/questions/2187474/
    int c;
    while ((c = getchar()) != '\n' && c != EOF);
}
...
void nhap(SV &sv){
    _flush();
    printf("\nNhap ten: "); fgets(sv.ten, 30, stdin);
    printf("\nNhap gioi tinh: "); fgets(sv.gt, 5, stdin);
...
void xuat(SV sv){
    printf("\nHo ten SV: %s", sv.ten);
    printf("Gioi tinh: %s", sv.gt);
    printf("Tuoi SV  : %d", sv.age);
